...
@hydra.main(config_path='../conf/sac2/default.yaml')
def main(conf: DictConfig):
    ...
    activation = nn.ELU() if conf.agent.activation == 'elu' else nn.ReLU()

    if conf.agent.use_layer_norm:
        if conf.agent.architecture == 'nn2':
            ...
            torch.nn.init.xavier_uniform_(policy[3].weight)
            torch.nn.init.xavier_uniform_(policy[5].weight, gain=conf.actor.output_scale)
            ...
            def make_q_func_with_optimizer():
                ...
                torch.nn.init.xavier_uniform_(q_func[4].weight)
                torch.nn.init.xavier_uniform_(q_func[6].weight)
                q_func_optimizer = torch.optim.Adam(q_func.parameters(), lr=conf.critic.lr)
                return q_func, q_func_optimizer
        elif conf.agent.architecture == 'nn3':
            policy = nn.Sequential(
                nn.Linear(obs_size, conf.actor.nn_size),
                nn.LayerNorm(conf.actor.nn_size),
                nn.Tanh(),
            ...
            torch.nn.init.xavier_uniform_(policy[0].weight)
            torch.nn.init.xavier_uniform_(policy[3].weight)
            torch.nn.init.xavier_uniform_(policy[5].weight)
            torch.nn.init.xavier_uniform_(policy[7].weight, gain=conf.actor.output_scale)
            policy_optimizer = torch.optim.Adam(policy.parameters(), lr=conf.actor.lr)
            ...
            def make_q_func_with_optimizer():
                ...
                torch.nn.init.xavier_uniform_(q_func[4].weight)
                torch.nn.init.xavier_uniform_(q_func[6].weight)
                torch.nn.init.xavier_uniform_(q_func[8].weight)
                q_func_optimizer = torch.optim.Adam(q_func.parameters(), lr=conf.critic.lr)
                return q_func, q_func_optimizer
        elif conf.agent.architecture == 'mpo':
            policy = nn.Sequential(
                nn.Linear(obs_size, conf.actor.nn_size),
                nn.LayerNorm(conf.actor.nn_size),
                nn.Tanh(),
        ...
        if conf.agent.architecture == 'nn2':
            ...
            torch.nn.init.xavier_uniform_(policy[6].weight, gain=conf.actor.output_scale)
            policy_optimizer = torch.optim.Adam(policy.parameters(), lr=conf.actor.lr)

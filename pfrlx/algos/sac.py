...
from pfrlx import utils
from pfrlx.algo import AttributeSavingMixin, BatchAlgo
from pfrlx.replay_buffer import ReplayUpdater, batch_experiences
...
class SoftActorCritic(AttributeSavingMixin, BatchAlgo):
    """Soft Actor-Critic (SAC).

    See https://arxiv.org/abs/1812.05905

    Args:
        conf : config of hydra
    ...
    def __init__(
        ...
        if self.entropy_target is not None:
            ...
            if conf.agent.alpha_lr is not None:
                self.temperature_optimizer = torch.optim.Adam(
                    self.temperature_holder.parameters(), lr=conf.agent.alpha_lr
                )
            else:
                self.temperature_optimizer = torch.optim.Adam(
    ...
    def _update_q_func(self, batch):
        ...
        with torch.no_grad(), utils.evaluating(self.policy), utils.evaluating(
            ...
            assert next_q1.shape == entropy_term.shape
    ...
    def _update_temperature(self, log_prob):
    ...
    def _update_policy_and_temperature(self, batch):
        ...
        assert entropy_term.shape == q.shape
        ...
        if self.entropy_target is not None:
            self._update_temperature(log_prob.detach())
